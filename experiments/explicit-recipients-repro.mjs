...
try {
  ...
  const planSource = readFileSync(join(REPO, 'docs/plan.md'), 'utf8')
  const readmeSource = readFileSync(join(REPO, 'README.md'), 'utf8')
  const readmeJaSource = readFileSync(join(REPO, 'README.ja.md'), 'utf8')
  ...
  check('現行planが明示宛先へ更新済み', planSource.includes('現行の通信契約は本節だけを正とする')
    && !planSource.includes('部位を跨ぐ合意・設計判断は全員宛で行う')
    && !planSource.includes('その席宛または全員宛の新着'))
  check('公開READMEがbroadcastを案内しない', !readmeSource.includes('all-addressed or DM')
    && !readmeJaSource.includes('全員宛も DM も'))
} finally {
  child.kill('SIGTERM')
