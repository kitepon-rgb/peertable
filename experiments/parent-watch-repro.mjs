...
try {
  ...
  const allBody = 't1-contract の最終試験を監査へ渡す。'
  await fetch(`${api}/messages`, {
    method: 'POST', headers: { 'content-type': 'application/json' },
  ...
  check('allは本文付きのroom更新eventとして親へ返す', allResult.status === 0
    && allEvent.type === 'parent_room_update'
    && allEvent.body === allBody
    && allEvent.to === 'all', allResult.stdout || allResult.stderr)
  check('all本文を親への直接出力へ載せる', allResult.stdout.includes(allBody), allResult.stdout)
