...
**未着手（次以降の campaign へ）**
- **teardown に「残して畳む」モードが無い（オーナー裁定 2026-08-09・次の筆頭課題）**。現行 `teardown.sh` は痕跡ゼロの一択で、**room の会話ログ（server 側の正本）も Lattice store（setup が作った場合）も消える**。ゲスト project を汚さない不可侵原則から来た設計だが、**自分の project で繰り返し使う時は「アーカイブして畳む」が要る**——2026-08-09 の refit 撤収では、**畳む直前に手でバックアップを取り、後から書き戻して commit する**という運用で凌いだ（`docs/archive/room-log_refit-20260808.md` と `.lattice/` がその成果物）。**手順が人の記憶に依存していて、忘れれば消える**。形の候補: `teardown.sh --archive` で ①room ログを `docs/archive/room-log_<plan_key>.md` へ書き出し ②`.lattice/` は削除せず残す（**git 追跡下に置く判断とセット**——refit では追跡へ入れた・196KB/31ファイル）③席と room 登録だけ撤去。**`--archive` を既定にするかは別問題**（ゲスト project では現行の痕跡ゼロが正しい）。**判定材料は `setup-state.json` の `lattice_preexisting` と、project が自分のものかどうか**
- **teardown が tmux 席を終了しない構成がある（2026-08-09 実測）**。`teardown done` まで完走し全段の `[実施]`/`[スキップ]` を出したのに、**`peer-*` の席5つが生きたまま残った**——**出力に「席終了」の段が1行も無い**ので、**やらなかったことが画面に出ない**（t6 が塞いだ型の、残っていた1箇所）。原因は未調査（席の列挙が `setup-state.json` 由来で、個別に `launch-seat.sh` した席と噛み合っていない可能性）。**当面は teardown 後に `tmux list-sessions | grep peer-` で残存確認**。恒久対処は**席終了の段を `[実施]`/`[スキップ]` つきで明示追加**すること。罠 DB: `peertable-teardown-sh-tmux-peer`
