...
- **決定64（2026-08-09・オーナー裁定）: 監査は2段——通常ToDoは軽く、重い監査はgateと契約クリティカルだけ**
  ...
  - **通常のToDo**: 実装者以外の**1席**が、対象diff・受入条件・focused testを**1巡**で確認して閉じる
  - **重い構造**: Wave/Phaseの受入gate（例: r4・s6・v2・t19級）と、契約クリティカル面
    （認可・lease発行・公開契約・データ破壊経路）だけ。r1のprecedence/lease充足への3巡は後者として正当
  - 迷ったら軽い方。重くする側に立証責任。監査を監査自身の指摘追加で増殖させない（親が終端記号を置く）
