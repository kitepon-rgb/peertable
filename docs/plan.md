...
- **決定71（2026-08-10・オーナー裁定）: DM宛先は「受け取って何をするか」に答えられる相手だけ——親宛は3種、席同士も同報を禁じる**

  brand-harvest卓（2026-08-10）で二つの穴が実測された。①メンバーが進捗・調整・意見・待機宣言まで
  親（Fable）宛DMに送り、高額な親のターンを1通ごとに消費した。②席同士でも「念のため共有」目的の
  同報DMが常態化し、宛先全席のターンとコンテキストを消費した。決定62・§16の明示宛先契約は「宛先を
  1人か配列で書けること」までしか決めておらず、「何を書いてよいか」は野放しだった——1発言＝宛先
  全席1ターンという課金構造（決定62）の下では、この野放しがそのままコストになる。

  オーナー裁定は次の最小化規律を課す:

  1. **親宛DMは3種類だけ。** ①done報告・監査受理要請、②オーナー承認gateに関わる物件、③親・オーナー
     にしか解けないblocker・裁定依頼（`[effort変更依頼]`はこれに該当）。進捗・調整・意見・待機宣言は
     親宛DMにしない——**待機宣言はroomへの平文宣言に変える**（`member.md`旧作法の「親宛DM」を訂正。
     親は決定68の運用側どおりroomログをpullして席数を判断しており、DMで起こされる必要はない）
  2. **席同士のDMも、その内容で今すぐ動く必要がある相手だけへ送る。** 同報・返答不要の報告・情報共有
     目的の複数宛先DMは禁止。**記録目的はroomログ（全員pull可読）で足りる**——決定62・§16が複数宛先
     配列を許したのは「2人以上に用がある発言」を機械的allへ倒さないためであって、「用がない相手も
     一緒に呼ぶ」ことを許した趣旨ではない
  3. **判断基準は「この宛先は受け取って何をする？」に答えられるかである。** 答えられない宛先は外す。
     一行の進捗共有・意見表明・情報提供は宛先を持たずroomへ流すか、必要なら自分宛にする

  影響: `skill/templates/member.md`・`skill/templates/member-standalone.md`・`skill/SKILL.md`の
