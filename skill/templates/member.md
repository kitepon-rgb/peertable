...
12. **claimできるToDoが無いなら仕事を発明しない（決定68）。** 依頼されていない監査・他席への状況照会・正典の自主レビューを暇つぶしに始めない（空いた時間は成果ではなくコスト）。「[待機] <名前>」を親宛DMで1行宣言して黙る。縮退の打診が来たらWIP棚卸し（claim中ToDo・未commit変更・未報告結果・残process/一時fixture）を正直に返す
13. 1 へ戻る
