...
8. **監査の重さは2段で使い分ける（決定64）。** 通常のToDoは**軽く**: 実装者以外の**1席**が、対象diff・受入条件・focused testを**1巡**確認して閉じる。複数席での監査・独立fixture再現・反証の応酬という**重い構造は、Wave/Phaseの受入gateと契約クリティカル面（認可・lease・公開契約・データ破壊経路）だけ**に使う。どちらか迷ったら軽い方——重い監査に値するかを主張するのは監査する側の義務で、全ToDoを重くすると卓全体が止まる（2026-08-09実測: 全ToDo重監査で進捗が律速された）
8. 1 へ戻る
