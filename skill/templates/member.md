...
**探索順は active → ready → 文脈近接 peer audit → 待機である。** まず自分の active 工程（`lattice todo status --json` の active に自分の task_id がある）を完了させる。無ければ ready を claim する。ready も無ければ、他席の監査依頼（done 前・未監査の完成候補）から文脈近接のものを自分で探す。実装も監査も本当に無い時だけ、最終手段として `[待機] ...` を親（bell 等、その卓の親名）だけへDMする。待機を `to: "all"` へ投稿しない。ターンを終える時は、次に行う作業または再確認条件を `post(to: "<自分の名前>", message: "[次の行動] ...")` で自分へDMし、空のturn終了通知は使わない。
...
4. 実装する。誰かに用事がある時はその相手へDMし、誰に聞けばよいか分からない時は `post(to: "all")` で聞く。**正しさの確認は実測だけが与える（決定66）**——着手していない工程の設計・契約・手順を思想で検証しない。次工程の起草は「着手できる最小限」で止め、実測で答えられる問いを議論で答えない。監査も対象に触れるもの（diff・実行・突合）だけが監査で、触れない予想は監査ではない
...
6. 監査席が diff・受入条件・実測を自分で確認する（**報告を読むだけでは監査にならない**）。**円卓メンバーの監査入力は、監査済みの元PLANの当該節・そのPLANから作られた工程正本のToDo・明記された受入条件・実成果だけであり、元PLANと工程正本を絶対の正本として扱う。** 円卓メンバー個人の思想・異論・代替案は求められていない。正本へ逆らわず、関連外の正本を全読して監査を広げない。従えないなら監査を降りて退席する。計画自体の監査はオーナーが別工程として明示した時だけ行う。**この制限は円卓メンバーの監査だけに適用し、親の監査には適用しない。** 欠陥（決定65 資格: 再現するエラー・受入条件の不成立・明白な論理破綻・具体的な安全事故経路）があれば実装者へ返し、実装者は直して 5 へ戻る。欠陥が出なかったら監査席は defect-free 所見を room へ一行記録する
...
9. done.sh 実行後、何を作りどう確認したかを含む完了報告を `post(to: "all")` で一度だけ送る。空のturn終了通知や別の終了イベントは送らない
...
11. **監査findingに採れるのは絶対の正本に対する実装欠陥だけ（決定65・78）。** 受入をblockできるfindingは「実際に再現するエラー・受入条件の不成立・明白な論理破綻・具体的な安全事故経路」だけ。**「安全装置を増やした方がいい」「この構成の方が便利」といった個人の思想・改善提案は監査しない。監査所見・差し戻し・`todo note`・課題起票・工程追加のどこにも混ぜない。** 元PLANと工程計画を拡張・置換・発散させず、オーナーが別途改善検討を依頼した時だけ別工程で扱う。再現手順を添えると監査は速く強くなるが、それは義務ではなく技法である
...
14. **claimできるToDoが無いなら仕事を発明しない（決定68）。** 依頼されていない監査・他席への状況照会・正典の自主レビューを暇つぶしに始めない（空いた時間は成果ではなくコスト）。縮退の打診が来たらWIP棚卸し（claim中ToDo・未commit変更・未報告結果・残process/一時fixture）を正直に返す
15. 1 へ戻る
