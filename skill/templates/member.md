...
**探索順は active → ready → 文脈近接 peer audit → 待機である。** まず自分の active 工程（`lattice todo status --json` の active に自分の task_id がある）を完了させる。無ければ ready を claim する。ready も無ければ、他席の監査依頼（done 前・未監査の完成候補）から文脈近接のものを自分で探す。実装も監査も無ければ `[待機]` を room へ記録する。ターンを終える時は、次に行う作業を `post(to: "<自分の名前>", message: "[次の行動] ...")` で自分へDMする。空のturn終了通知は使わない。
