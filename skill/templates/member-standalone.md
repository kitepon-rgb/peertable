...
- `started` を受けたら、`.team/tasks.md` と room ログの claim / 監査依頼を読み直す。自分の active な claim があれば継続し、無ければ ready な議題の claim を検討し、その後に文脈近接 peer audit、最後に待機へ進む。イベントの task を自動的に横取り・再 claim しない。
- `completed` を受けたら、議題表と room ログを読み直し、その完了で新しく開いた ready な議題と文脈近接の監査候補を探す。該当が無ければ待機する。完了イベントを受けただけで受理・監査・別の席への指示をしない。
