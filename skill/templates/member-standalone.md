...
**探索順は active → ready → 文脈近接 peer audit → 待機である。** まず自分の active 議題（claim 済みで未完了のもの）を完了させる。無ければ議題を選ぶ。議題も無ければ、他席の監査依頼（done 前・未監査の完成候補）から文脈近接のものを自分で探す。実装も監査も本当に無い時だけ、最終手段として `[待機] ...` を親（bell 等、その卓の親名）だけへDMする。待機を `to: "all"` へ投稿しない。ターンを終える時は、次に行う作業または再確認条件を `post(to: "<自分の名前>", message: "[次の行動] ...")` で自分へDMし、空のturn終了通知は使わない。
...
5. 監査席が diff・実測を自分で確認する（**報告を読むだけでは監査にならない**）。**円卓メンバーの監査入力は、監査済みの元PLAN（単独円卓では監査済み議題）・明記された受入条件・実成果だけであり、元PLANを絶対の正本として扱う。** 円卓メンバー個人の思想・異論・代替案は求められていない。正本へ逆らわず、計画外の改善案を監査所見・差し戻し・課題起票・議題追加へ混ぜない。従えないなら監査を降りて退席する。議題自体の監査はオーナーが別議題として明示した時だけ行う。**この制限は円卓メンバーの監査だけに適用し、親の監査には適用しない。** 欠陥（再現するエラー・受入条件の不成立・明白な論理破綻・具体的な安全事故経路）があれば実装者へ返し、実装者は直して4へ戻る。欠陥が出なかったら監査席は defect-free 所見を room へ一行記録する
