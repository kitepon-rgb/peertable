...
1. **聞き取り**: 対象プロジェクトのパス / **工程正本（`Lattice 併用`＝既定 / `単独`）** / メンバー数とモデル・effort（モデル既定: Sonnet。**effort は既定値を置かない——席ごとに必ず決める**。`launch-seat.sh` の effort は必須引数で、未指定は着席前に非ゼロ終了する。モデル選定は作業の性質——設計か確定実装か——を軸にする。決定49）/ 初期タスク群（何を作るか）/ room 名（既定: プロジェクトのディレクトリ名）/ **公開URL基底**（Lattice 併用のみ。外部ペインに書く URL。クオ環境は `https://peertable.kitepon.dev`。未指定なら room サーバーの URL がそのまま入る＝LAN URL は Lattice を外から見た時に開けない）
   ...
   - **運用中の席数の標準は「ready＋activeな実装ToDo数」（決定68）**: 起動時の幅は計画値、この式は生きた実測値で、campaignが進めば自然に縮む。標準を上回った席は親が縮退する。監査・受入は実装席が兼ねる（決定67）ので監査要員を席数に数えない。claimできるToDoが無い席は仕事を発明せず（依頼されていない監査・照会・自主レビュー禁止）、最終手段として親だけへ待機DMして黙るか親に畳まれる。待機を`to: "all"`へ投稿しない
...
- **監査は卓の中で行う。親はコードを読まない**（決定60）。完了 task の監査は**実装者以外の席**が実物（diff・検証結果・ハーネスの正負両方）を読んで行い、**所見を room へ出す**。親がするのは、その所見を読んで**受理を宣言すること**だけである。
  - **円卓メンバーの実装監査では、監査済みの元PLANと、そのPLANから作られた工程正本のToDo・明記された受入条件を絶対の正本として扱う（決定78）。** 円卓メンバー個人の思想・異論・代替案は求められておらず、監査へ持ち込まない。監査者は当該節と実成果だけを突合し、正本へ逆らわず、計画外の改善案を監査所見・差し戻し・`todo note`・課題起票・工程追加へ混ぜない。従えない監査者は監査を降りて退席する。計画自体の監査はオーナーが別工程として明示した時だけ行う。**この制限は円卓メンバーの監査だけに適用し、親の監査には適用しない**
  ...
  - 監査所見に書くのは、絶対の正本に対して再現した実装欠陥だけである。受入条件外の思想・改善案は「申し送り」と言い換えて残さず、監査のどの記録面にも書かない
