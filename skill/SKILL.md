...
6.6 **容量差分ブリッジ（parent-join.sh が親登録後に自動で起こす）**: `ensure-bridge.sh <project> capacity` がLatticeのstatus＋plan別independence（単独卓は`.team/tasks.md`＋room claim）とroom membersを観測する。親・deadを除くlive workerとの差が変わった時だけ、親とidle reclaim席へ一通の`[capacity] PEERTABLE_CAPACITY_CHANGED` DMを送る。状態は`.team/capacity-advisor.json`、process記録は`.team/capacity-bridge.json`、ログは`.team/capacity-bridge.log`。停止は`node scripts/capacity-bridge.mjs <project> --stop`（teardownが自動で行う）。setup時点では親が未登録なので起こさない。
