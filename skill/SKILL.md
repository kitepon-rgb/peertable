...
- **親の再着卓**（context が要約された／セッションが替わった時。決定51 のメンバー版に対応する親版。2026-08-08 実測）: 卓は生きたまま親だけが記憶を失う局面なので、**復帰は記憶ではなく正本から取り直す**。順に:
  ...
  3. **member 登録は残っている**ので `parent-join.sh` を再実行しない（実測: 親の登録はセッションを跨いで残る。再実行すると `<名前> が参加した` が二重に流れて、席の側に「新しい親が来た」と誤読される）。`curl -s $URL/api/$ROOM/members` で自分の名前を確認するだけでよい
  4. **SSE を張り直す**——Monitor で `curl -sN $URL/api/$ROOM/events`。**前の Monitor は死んでいる**ので、張り直さないと以後の新着に気づかない
...
- **`claude-in-chrome` の呼び出しは返らないことがある**。原因は2種で、解き方が違う（2026-08-08 に席1つが9分半沈黙して実測）:
  ...
  - **親の督促手順に組み込む**: 報告途絶を見つけたら ①`capture-pane` で実状態（`Cogitating…` か・ダイアログか・`pane_dead`）②ダイアログなら `pty_key` で解除 ③解除できない型ならオーナーへ。**呼びかけを増やすのは①の前にやらない**——ターン中なら読まれないので遅くなるだけ
