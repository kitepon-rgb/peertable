...
- **自作製品の npm publish と本番反映は完遂に含める。** 既定ブランチの祖先であることは `verify-release-commit` が強制する。force 系・履歴改変は目的・影響・戻し方を説明してから行い、復旧反射では使わない。
...
├── docs/plan.md          # 計画書（設計・決定履歴の正本）。決定は通し番号（現在 91 まで）
